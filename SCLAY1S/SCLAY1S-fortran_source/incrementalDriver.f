...
! February 2009: *ObeyRestrictions works with *Repetitions 
       ...
       use unsymmetric_module     !module defined in usolve.f
       implicit none              !pour pas que le type des variables soit specifie par à l aide de la première lettre de leurs noms
       character*80  cmname,rebarn   !définition de variables contenant au maximum 80 characters
       integer ndi,nshr,ntens,nstatv,nprops,ncrds  !définition des entiers
       !ndi:
       
       integer noel,npt,layer,kspt,lrebar,kinc,i
       real(8),parameter,dimension(3,3):: delta =
     &                          reshape((/1,0,0,0,1,0,0,0,1/),(/3,3/)) !On definit une matrice 3,3 avec des constantes qui ne changeront pas,
       !le (8) signifie un reel à double precision, la fonction reshape permet de construire la matrice à partir des données formulées en ligne
    ...
    !      parameter( noel=1 ,npt=1,layer=1,kspt=1,lrebar=1)
     ...
     &  coords(ncrds),drot(3,3),dfgrd0(3,3),dfgrd1(3,3)
      ...
      !ddsdde:Jacobian matrix 
      character(40) :: keywords(10), outputfilename, parametersfilename,
     &       initialconditionsfilename, testfilename, outputfilename1
      !string added by jpgras
      character(14) :: directory
      character(21) :: finalname
      !end change
     ...
     &      dstran_Cart(6), ddsdde_bar(6,6), deltaTime
      real(8),parameter :: sq3=1.7320508075688772935d0,   !racine de 3
     &                     sq6=2.4494897427831780982d0,   !racine de 6
     &                     sq2=1.4142135623730950488d0,   !racine de 2
     ...
     &             dfgrd0(3,3), dfgrd1(3,3),deltaTime
        character(40) :: keyword2, keyword3
...
!  read the material parameters ************************************************
      ...
      continue
      !change made by jp gras
      !parametersfilename = 'parameters.inp'
      !initialconditionsfilename  = 'initialconditions.inp'
      !testfilename = 'test.inp'
      parametersfilename = 'Running_files/parameters.inp'
      initialconditionsfilename  = 'Running_files/initialconditions.inp'
      testfilename = 'Running_files/test.inp'      
      outputfilename = '--'
      verbose = .true.
...
!  initialize everything
      open(1,err=902,file=initialconditionsfilename)
      read(1,*) ntens_in
      stress(:) = 0.0d0
      time(:) = 0.0d0
      stran(:)=0.0d0
 ...
 ! read the loading path
      open(1,err=903,file=testfilename)
      read(1,'(a)') aLine
        i = index(aLine,'#')
        if(i==0) then
      ...
      !change made by jpgras, put results in another directories
      !open(2,err=904,file=outputfilename)
      directory = 'Running_files/'
      finalname = directory // outputfilename
      open(2,err=904,file=finalname)
      !end change
      
      !!!!I move this part after the initialization in UMAT
      !!!!!!!!!Déplacement de cette partie après l initialisation dans UMAT
!      write(2,'(a10,50a17)') 'time(1)', 'time(2)',
!     &'stran(1)','stran(2)','stran(3)','stran(4)','stran(5)','stran(6)',
!     & 'stress(1)','stress(2)','stress(3)','stress(4)','stress(5)',
!     & 'stress(6)','alpha_lat','alpha_vert','alpha_lat','alpha_12',
!     & 'alpha_23','alpha_13','alpha0','pmi','X0',
!     & 'pm0','e0','initializ','Pp0',
!     &'Peq0','OCR','dPeqdP','statev(17)'
...
!     &                                stran, stress, statev
  ...
  10  keywords(3) = trim(keywords(3))
     ...
     &                                                     maxiter = 1  ! no iterations are necessary
      ...
      !Rajout: Appel de UMAT uniquement pour l initialisation des variables détats
      !Dans incrémental driver original, les variables d´états sont lues dans un fichier input
      !Le programme de Siva calcule lui les variables d´états à partir du fichier de paramètres en appelant UMAt
      !Il faut donc appeler UMAT une fois de plus pour línitialisation
      
      if (time(2) == 0) then
          call  UMAT(stress,statev,ddsdde,sse,spd,scd,                    ! \com  first call  with dstrain=0 dtime=0 just for stiffness
     &    rpl,ddsddt,drplde,drpldt,
     &    stran,dstran,time,dtime,temp,dtemp,predef,dpred,cmname,
     &    ndi,nshr,ntens,nstatv,props,nprops,coords,drot,pnewdt,
     &    celent,dfgrd0,dfgrd1,noel,npt,layer,kspt,0,kinc)                !  \com  kStep=0
      
          write(2,'(a10,50a17)') 'time(1)', 'time(2)',
     &    'stran(1)','stran(2)','stran(3)','stran(4)','stran(5)',
     &    'stran(6)',
     &    'stress(1)','stress(2)','stress(3)','stress(4)','stress(5)',
     ...
     &    'statev(14)','statev(15)','statev(16)','statev(17)...'
          if(heading(1:1) /= '#') write(2,*) trim(heading)
          write(2,'(500(g15.8,3h    ))') time+(/dtime,dtime/),
     &                                stran, stress, statev
      endif
      
      !Fin du rajout!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      
!     CALCULATE EACH STEP AS A SEQUENCE OF INCREMENTS  with the preliminary zero-load  call to get the 1st stiffness
        dstran=0
        dtime=0
        dtemp=0
        kinc=0
!       kStep=0 
     ...
     &    celent,dfgrd0,dfgrd1,noel,npt,layer,kspt,kStep,kinc)
      ...
      !   if(kinc==252 .and. kiter==5) then !  for debugging with Mma only
      !   call write66(ddsdde_bar)
      !   call write6(u_dstress)
      !   call write6(c_dstran)
      !   endif
...
94    continue
      !if((kiter==maxiter) .and. mod(kinc,100)==0 .and. verbose ) then    ! write to screen
 ...
 95   continue !--------------------end of iteration
     ...
     & '||u_dstress|| too large.'                 ! check  the Rosc.stress error < toler
      ...
      ! chop to prevent fortran error writing 1.3E-391
 ...
 904  stop 'I cannot open outputfile'
      ...
      subroutine get_command_line_arguments()  ! file names in the command line override defaults
      integer :: iarg,narg,inputLength,is, length,iargc
      integer, parameter :: argLength=40
      character(argLength) :: anArgument, argType, argValue
      narg = iargc()
      do iarg = 1,narg
        call getarg(iarg,anArgument)
!        if(length<0) stop 'error while reading a command line argument'
        is = index(anArgument,'=')
      ...
      subroutine write66(a) ! for debugging with Mma
      ...
      subroutine write6(a) ! for debugging with Mma
